...
object ClimateService {
  ...
  val climateRelatedWords = Set("global warming","IPCC","climate change")
  climateRelatedWords.exists(word => description.contains(word))
}
...
def filterDecemberData(list: List[Option[CO2Record]]): List[CO2Record] = {
  list.flatMap {
    case Some(record) if record.month != 12 => Some(record)
    case _ => None
  }
}
...
def getMinMaxByYear(list: List[CO2Record], year: Int): (Double, Double) = {
  ...
  if (filteredList.isEmpty) {
    throw new IllegalArgumentException(s"No data available for the year $year")
  } else {
    val ppmValues = filteredList.map(_.ppm)
    (ppmValues.min, ppmValues.max)
  }
}
